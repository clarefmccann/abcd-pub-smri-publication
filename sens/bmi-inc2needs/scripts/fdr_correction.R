...
p_f <- read.csv(paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_combined_pvals.csv")) %>% 
  filter(sex == "f",
...
p_m <- read.csv(paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_combined_pvals.csv")) %>% 
  filter(sex == "m",
         !(region %in% c("suprateialv", "wholeb", "allventricles", "intracranialv", 
                                   "latventricles", "subcorticalgv", "meanlh", "meanrh", 
                                   "mean", "total", "totalrh", "totallh")))
...
best_fit_timing <- read.csv(paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_best_fits.csv")) %>%
...
best_fit_tempo <- read.csv(paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_best_fits.csv")) %>%
...
write.csv(merged, file = paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_fdr_corrected_p.csv"))
...
write.csv(timing_pvals, file = paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_timing_pvals.csv"))
write.csv(tempo_pvals, file = paste0(proj_root, "sens/bmi-inc2needs/results/cleaned/combat_tempo_pvals.csv"))
