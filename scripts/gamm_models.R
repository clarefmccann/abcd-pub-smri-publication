...
formula_age <- as.formula(paste("value ~ s(age_scaled, bs = 'cs', k = 7) + site_id_l"))
formula_timing_main <- as.formula(paste("value ~ timing_scaled + s(age_scaled, bs = 'cs', k = 7) + site_id_l"))
formula_tempo_main <- as.formula(paste("value ~  tempo_scaled + s(age_scaled, bs = 'cs', k = 7) + site_id_l"))
formula_timing_int <- as.formula(paste("value ~ timing_scaled + s(age_scaled, bs = 'cs', k = 7) + s(age_scaled, by = timing_scaled, k = 7, bs = 'cs') + site_id_l"))
formula_tempo_int <- as.formula(paste("value ~ tempo_scaled + s(age_scaled, bs = 'cs', k = 7) + s(age_scaled, by = tempo_scaled, k = 7, bs = 'cs') + site_id_l"))
...
all_results_csv <- paste0(root, "results/all_results_", region_name, "_", sex, "_combat.csv")
...
par(mfrow = c(2,2))
print(paste("printing gam.check for the age_timing_int model for", sex, region_name))

png(paste0(root, "model_dx/gam_age_check_plots_",region_name,"_",sex,"_combat.png"))
gam.check(age_model$gam)

grid::grid.text(paste("page 1: age, page 2: age & timing int, page 3: age & tempo int\nregion:", region_name, "\nsex:", sex),
                x = 0.5, y = 0.95, gp = grid::gpar(fontsize = 11, fontface = "bold", col = "blue"),
                just = "center")

print(paste("printing gam.check plots for the age_tempo model for", sex, region_name))
png(paste0(root,"model_dx/gam_age_timing_check_plots_",region_name,"_",sex,"_combat.png"))
gam.check(age_timing_int_model$gam)

print(paste("printing gam.check plots for the age_timing_tempo model for", sex, region_name))
png(paste0(root, "model_dx/gam_age_tempo_check_plots_",region_name,"_",sex,"_combat.png"))
gam.check(age_tempo_int_model$gam)

dev.off()

print("--------------------------------------------------")

print("saving effects predicted dfs")
# 
# ---- seeds & centroids ----
set.seed(90025)

kmeans_result_timing <- kmeans(timing_main_data_region$timing_scaled, centers = 3)
time_centroids <- as.vector(kmeans_result_timing$centers)

kmeans_result_tempo <- kmeans(timing_main_data_region$tempo_scaled, centers = 3)
tempo_centroids <- as.vector(kmeans_result_tempo$centers)

# ---- reference values (hold constant) ----
ref_site <- timing_main_data_region$site_id_l[1]

# ---- age sequence for smooth curves ----
age_seq <- seq(min(timing_main_data_region$age_scaled, na.rm = TRUE),
               max(timing_main_data_region$age_scaled, na.rm = TRUE),
               length.out = 150)

# ======================================================================
# TIMING models — FULL surface (main + interaction)
# ======================================================================

pred_full_timing <- tidygam::predict_gam(
  age_timing_int_model$gam,
  values = list(
    age_scaled    = age_seq,
    timing_scaled = as.numeric(time_centroids),
    site_id_l     = ref_site
  )
)

output_full_timing_csv <- paste0(root, "int_effect/int_effect_full_timing_",
                                 brain, "_", region_name, "_", sex, "_combat.csv")
print(paste("saving full surface (age × timing) for", region_name, "to", output_full_timing_csv))
write.csv(pred_full_timing, file = output_full_timing_csv, row.names = FALSE)

# ======================================================================
# TIMING models — INTERACTION-ONLY (strip s(age) and linear timing)
# ======================================================================

pred_interaction_timing <- tidygam::predict_gam(
  age_timing_int_model$gam,
  values = list(
    age_scaled    = age_seq,
    timing_scaled = as.numeric(time_centroids),
  ...
  exclude_terms = c("s(age_scaled)", "timing_scaled")
)

output_intonly_timing_csv <- paste0(root, "int_effect/int_effect_alone_timing_",
                                    brain, "_", region_name, "_", sex, "_combat.csv")
print(paste("saving interaction-only (age × timing) for", region_name, "to", output_intonly_timing_csv))
write.csv(pred_interaction_timing, file = output_intonly_timing_csv, row.names = FALSE)

# ======================================================================
# TEMPO models — FULL surface (main + interaction)
# ======================================================================

pred_full_tempo <- tidygam::predict_gam(
  age_tempo_int_model$gam,
  values = list(
    age_scaled   = age_seq,
    tempo_scaled = as.numeric(tempo_centroids),
    site_id_l    = ref_site
  )
)

output_full_tempo_csv <- paste0(root, "int_effect/int_effect_full_tempo_",
                                brain, "_", region_name, "_", sex, "_combat.csv")
print(paste("saving full surface (age × tempo) for", region_name, "to", output_full_tempo_csv))
write.csv(pred_full_tempo, file = output_full_tempo_csv, row.names = FALSE)

# ======================================================================
# TEMPO models — INTERACTION-ONLY (strip s(age) and linear tempo)
# ======================================================================

pred_interaction_tempo <- tidygam::predict_gam(
  age_tempo_int_model$gam,
  values = list(
    age_scaled   = age_seq,
    tempo_scaled = as.numeric(tempo_centroids),
  ...
  exclude_terms = c("s(age_scaled)", "tempo_scaled")
)

output_intonly_tempo_csv <- paste0(root, "int_effect/int_effect_alone_tempo_",
                                   brain, "_", region_name, "_", sex, "_combat.csv")
print(paste("saving interaction-only (age × tempo) for", region_name, "to", output_intonly_tempo_csv))
write.csv(pred_interaction_tempo, file = output_intonly_tempo_csv, row.names = FALSE)

# ======================================================================
# TIMING models — MAIN EFFECT
# ======================================================================

pred_main_timing <- tidygam::predict_gam(
  age_timing_main_model$gam,
  values = list(
    age_scaled    = age_seq,
    timing_scaled = as.numeric(time_centroids),
    site_id_l     = ref_site
...
output_main_timing_csv <- paste0(root, "main_effect/main_effect_timing_",
                                 brain, "_", region_name, "_", sex, "_combat.csv")
dir.create(dirname(output_main_timing_csv), showWarnings = FALSE) # Create directory if it doesn't exist
print(paste("saving main effect (age + timing) for", region_name, "to", output_main_timing_csv))
write.csv(pred_main_timing, file = output_main_timing_csv, row.names = FALSE)

# ======================================================================
# TEMPO models — MAIN EFFECT
# ======================================================================

pred_main_tempo <- tidygam::predict_gam(
  age_tempo_main_model$gam,
  values = list(
    age_scaled   = age_seq,
    tempo_scaled = as.numeric(tempo_centroids),
    site_id_l    = ref_site
...
output_main_tempo_csv <- paste0(root, "main_effect/main_effect_tempo_",
                                brain, "_", region_name, "_", sex, "_combat.csv")
dir.create(dirname(output_main_tempo_csv), showWarnings = FALSE)
print(paste("saving main effect (age + tempo) for", region_name, "to", output_main_tempo_csv))
write.csv(pred_main_tempo, file = output_main_tempo_csv, row.names = FALSE)

# ======================================================================
# AGE-ONLY model
# ======================================================================

pred_age_only <- tidygam::predict_gam(
  age_model$gam,
  values = list(
    age_scaled = age_seq,
    site_id_l  = ref_site
...
output_age_only_csv <- paste0(root, "age_effect/age_effect_",
                              brain, "_", region_name, "_", sex, "_combat.csv")
dir.create(dirname(output_age_only_csv), showWarnings = FALSE)
print(paste("saving age-only effect for", region_name, "to", output_age_only_csv))
write.csv(pred_age_only, file = output_age_only_csv, row.names = FALSE)
