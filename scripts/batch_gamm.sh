...
for brain in "${brains[@]}"; do
  for sex in "${sexes[@]}"; do
    ...
    for region in "${regions[@]}"; do
      job_name="combat_gamm_${brain}_${sex}_${region}"
      output_log="/u/home/c/clarefmc/projects/smri-pub-abcd/out/${job_name}.out"
      error_log="/u/home/c/clarefmc/projects/smri-pub-abcd/error/${job_name}.err"
      
      cmd="qsub -cwd -N combat_gamm_${brain}_${sex}_${region} -o "$output_log" -e "$error_log" -l h_data=60G,h_rt=24:00:00 -v brain='$brain',sex='$sex',region='$region' run_R_bam.sh"
